...
public class Test2 : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        var rayStartPoint = new Vector3(5.0f, tapWorldPoint.y, 0.0f);

        RaycastHit hit;
        if (volumeRayCast && !isPull)
        {
            if (!Physics.SphereCast(rayStartPoint, tapRadius, Vector3.left, out hit))
            {
                return;
            }
        ...
        {
            if (!Physics.Raycast(rayStartPoint, Vector3.left, out hit))
            {
                return;
            }
        }
    ...
    public void DeformAtPoint(in Vector3 inHitPoint, bool inIsPull, bool updateMesh = true)
    {
        ...
        var center = new Vector3(0.0f, hitPoint.y, 0.0f);
        ...
        for (int i = 0; i < softVerts.Length; i++)
        {
            ...
            var angle = Vector3.SignedAngle(centerToHit, centerToLocal, Vector3.up);
            ...
            if (tmpDist < maxDist)
            {
                ...
                softVerts[i] = new Vector3(localVert.x+ add.y, localVert.y + add.y, localVert.z + add.z);
            }
        }
